<template>
  <div class="authentication-page">
    <el-card>
      <label class="authentication-title">Регистрация</label>

      <div class="segmented-container">
        <el-segmented v-model="selectedAuthType" :options="['Авторизация', 'Регистрация']" @change="this.$router.push({ name: 'login' })" style="width: 250px" />
      </div>

      <el-form :model="form" :rules="rules" ref="formRef" hide-required-asterisk @submit.prevent="handleRegister">
        ...
        <el-form-item label="Повторите пароль" prop="repeatPassword" label-position="top" style="margin-top: 10px;">
          <el-input style="width: 250px;" v-model="form.repeatPassword" type="password" placeholder="Повторите ваш пароль" clearable required />
        </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        repeatPassword: [
          { required: true, message: 'Пожалуйста, повторите ваш пароль', trigger: 'blur' },
          {
  ...
  methods: {
    async handleRegister() {
      try {
        ...
        if (userExists) {
          ElMessage.error('Пользователь с указанным email уже существует');
          return;
        }

        const registerResponse = await ApiClient.post('/auth/register', this.form);
